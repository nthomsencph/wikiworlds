...
def test_authenticate_user(session: Session) -> None:
    ...
    user_in = UserCreate(email=email, password=password)
    user = db.create_user(session=session, user_create=user_in)
    authenticated_user = db.authenticate(
...
def test_not_authenticate_user(session: Session) -> None:
    email = random_email()
    password = random_lower_string()
    user = db.authenticate(session=session, email=email, password=password)
    assert user is None
...
def test_get_user(session: Session) -> None:
    ...
    user = db.create_user(session=session, user_create=user_in)
    user_2 = db.get_user_by_id(session=session, user_id=user.id)
    assert user_2
...
def test_update_user(session: Session) -> None:
    ...
    if user.id is not None:
        db.update_user(session=session, db_user=user, user_in=user_in_update)
    user_2 = db.get_user_by_id(session=session, user_id=user.id)
    assert user_2
    assert user.email == user_2.email
