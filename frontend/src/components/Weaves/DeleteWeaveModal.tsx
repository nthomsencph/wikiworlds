    ...
    const deleteMutation = useMutation({
        ...
        onError: (error: any) => {
            toaster.create({
                ...
                description: error.message || "An error occurred while deleting the weave.",
                type: "error",
            })
            setIsDeleting(false)
        },
                    ...
                    {isOnlyWeave ? (
                        <Box>
                            <Text color="gray.300" mb={4}>
                                A creator must have at least one weave. Since this is your only one, you can&apos;t delete it. Create a new weave before deleting this one, or keep using {currentWeave?.name}.
                            </Text>
                        </Box>
                    ) : (
                        <Box>
